...
func New(width, height int) *Texture {
	return &Texture{
		width:  width,
		height: height,
		Pixels: make([]color.Pixel565, width*height),
	}
}

// SetPixel in the texture.
func (tx *Texture) SetPixel(x, y int, c color.Pixel565) {
	tx.Pixels[y*tx.width+x] = c
}

// GetPixel from the texture.
func (tx *Texture) GetPixel(x, y int) color.Pixel565 {
...
func Load(name string) (*Texture, error) {
	...
	for y := 0; y < b.Y; y++ {
		for x := 0; x < b.X; x++ {
			...
			tx.Pixels[y*tx.width+x] = color.New(r>>8, g>>8, b>>8)
		}
	}
	return tx, nil
}
